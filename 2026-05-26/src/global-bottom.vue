...
const currentSlideRoute = computed(() => slidev?.nav.currentSlideRoute)
...
function usePloy(number = 16) {
  function getPoints(): Range[] {
    ...
    const rng = seedrandom(`${seed.value}-${currentSlideRoute.value?.no ?? 1}`)
...
<template>
  <div
    class="bg transform-gpu overflow-hidden pointer-events-none opacity-50"
    :style="{ filter: `blur(70px) hue-rotate(${hue}deg)` }"
    aria-hidden="true"
  >
    <div
      class="clip bg-gradient-to-r from-[#DDDDDD] to-white/10"
